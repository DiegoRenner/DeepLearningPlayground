...
class Pinns:
    ...
    def add_temporal_boundary_points(self):
        ...
        #print(torch.cat([input_tb[:,0].reshape(input_tb.shape[0],1),input_tb[:,1].reshape(input_tb.shape[0],1)], 0).shape, output_tb)
    ...
    def add_spatial_boundary_points(self):
        ...
        #denom_L = torch.ones((input_sb_L.shape[0],1)) + torch.exp(-200*(input_sb_L[:,0]-0.25*torch.ones((input_sb_L.shape[0],1))))
    ...
    def compute_loss(self, inp_train_sb, u_train_sb, inp_train_tb, u_train_tb, inp_train_int, verbose=True):
        ...
        #print(u_pred_sbf_grad)
        #u_pred_sb[n_sb:2*n_sb, :] = torch.autograd.grad(u_pred_sb[n_sb:2*n_sb, :].sum(), inp_train_sb[n_sb:2*n_sb,1],create_graph=True)[0]
        #u_pred_sb[2*n_sb:3*n_sb, :] = torch.autograd.grad(u_pred_sb[2*n_sb:3*n_sb, :].sum(), inp_train_sb[2*n_sb:3*n_sb,1],create_graph=True)[0]
        #u_pred_sb[3*n_sb:4*n_sb, :] = torch.autograd.grad(u_pred_sb[3*n_sb:4*n_sb, :].sum(), inp_train_sb[3*n_sb:4*n_sb,1],create_graph=True)[0]
        #u_pred_tb = self.apply_initial_condition(inp_train_tb)
