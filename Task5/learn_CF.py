...
batch_size = n_data
training_set = DataLoader(torch.utils.data.TensorDataset(input_data, output_data), batch_size=batch_size, shuffle=True)
...
def run_configuration(conf_dict, x, y):
    ...
    history = fit(my_network, training_set, x_val, y_val, n_epochs, optimizer_,
            p=2, verbose=False)
    ...
    prediction_t_tensor = input_data#torch.from_numpy(prediction_t)
...
network_properties = {
    ...
    "batch_size": [n_data,int(np.floor(n_data/10)), int(np.floor(n_data/2)), n_data], #batch_size=1 to expensive
