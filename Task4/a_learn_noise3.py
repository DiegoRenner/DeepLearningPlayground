...
def model_noise(x_observed, u_observed):
    ...
    mean = model(x_observed)
    print(mean)
    prior_sigma = dist.Normal(torch.zeros((n_samples,1)).float(),torch.from_numpy(np.ones((n_samples,1))).float())
    sigma = pyro.sample("sigma", prior_sigma)
    print(sigma)
    #sigma_likelihood = G(inputs)
    #print(mean,sigma_likelihood)
    likelihood = dist.Normal(mean, sigma)
...
posterior = MCMC(nuts_kernel, num_samples=n_samples_nuts, warmup_steps=1000,
        initial_params={"sigma": torch.from_numpy(np.arange(0,0.5,1/n_samples))})
